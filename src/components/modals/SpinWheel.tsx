...
interface SpinPrize {
  ...
  emoji: string | null; image_url: string | null; sound_url: string | null;
  animation_url: string | null; weight: number;
}
...
const SpinWheel = ({ open, onOpenChange, onReward }: SpinWheelProps) => {
  const { user } = useAuth();
  ...
  const fetchData = async () => {
    ...
    if (user) {
      ...
      const { count } = await supabase.from("user_spins").select("id", { count: "exact", head: true })
        .eq("user_id", user.id).gte("spun_at", today.toISOString());
  ...
  const spin = async () => {
    ...
    if (spinsToday >= maxSpins) { toast.error(`You've used all ${maxSpins} spins today!`); return; }
    ...
    setTimeout(async () => {
      setResult(prize); setSaving(true);
      if (prize.sound_url) { try { audioRef.current = new Audio(prize.sound_url); audioRef.current.play(); } catch {} }

      await Promise.all([
        supabase.from("user_spins").insert({ user_id: user.id, prize_id: prize.id, reward_amount: prize.value }),
        supabase.from("transactions").insert({ user_id: user.id, type: "spin_reward", amount: prize.value, description: `Spin Wheel: ${prize.label}` }),
        supabase.rpc("add_xp", { p_user_id: user.id, p_amount: Math.floor(prize.value) }),
      ]);

      setSpinsToday(prev => prev + 1);
      setSaving(false); setSpinning(false);
        ...
        {loading ? (
          ...
          <>
            <div className="relative w-64 h-64 mx-auto my-4">
              ...
              <motion.div className="w-full h-full rounded-full relative overflow-hidden border-4 border-primary/30"
                animate={{ rotate: rotation }} transition={{ duration: 4, ease: [0.17, 0.67, 0.12, 0.99] }}>
                <svg viewBox="0 0 200 200" className="w-full h-full">
                  {prizes.map((seg, i) => {
                    const startAngle = i * segmentAngle; const endAngle = startAngle + segmentAngle;
                    const startRad = (Math.PI / 180) * (startAngle - 90); const endRad = (Math.PI / 180) * (endAngle - 90);
                    ...
                    const tx = 100 + 65 * Math.cos(midRad); const ty = 100 + 65 * Math.sin(midRad);
                    return (
                      <g key={seg.id}>
                        <path d={`M100,100 L${x1},${y1} A100,100 0 ${largeArc},1 ${x2},${y2} Z`} fill={seg.color} stroke="hsl(230, 25%, 7%)" strokeWidth="1" />
                        {seg.image_url ? (
                          <image href={seg.image_url} x={tx - 12} y={ty - 12} width="24" height="24" clipPath="circle(12px)" />
                        ) : (
                          <text x={tx} y={ty} textAnchor="middle" dominantBaseline="middle" fill="white" fontSize="12" fontWeight="bold"
                            transform={`rotate(${(startAngle + endAngle) / 2}, ${tx}, ${ty})`}>{seg.label}</text>
                        )}
                      </g>
                    );
                  })}
            ...
            <AnimatePresence>
              {result && (
                <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-2">
                  {result.image_url ? <img src={result.image_url} alt={result.label} className="w-16 h-16 mx-auto rounded-lg object-cover" />
                   : result.animation_url ? <img src={result.animation_url} alt={result.label} className="w-16 h-16 mx-auto" />
            ...
            <Button onClick={spin} disabled={spinning || saving || remaining <= 0} className="w-full gradient-primary text-white border-0 gap-2">
              {spinning ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Star className="h-4 w-4" /> Spin! ({remaining} left)</>}
            </Button>
          </>
        )}
      </DialogContent>
